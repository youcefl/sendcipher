...
impl ChunkGenerator for RandomChunkGenerator {
    ...
    fn process_data(&mut self, data: &[u8]) -> Vec<Chunk> {
        ...
        while remaining_data_offset != data.len() {
            let mut available_space = if self.chunks.is_empty() { 0u64 } else { 
            ...
            let copyable_amount = ((data.len() - remaining_data_offset) as usize).min(available_space as usize);
            let last_chunk = self.chunks.last_mut().unwrap();
            last_chunk.data_mut().extend_from_slice(&data[remaining_data_offset..remaining_data_offset + copyable_amount]);
        ...
        self.chunks.drain(0..ready_count).collect()
...
impl RandomChunkGenerator {
    ...
    pub fn new(chunking_threshold: u64, min_chunk_size: u64, max_chunk_size: u64) -> Self {
        Self{
            span_generator: SpanGenerator::new(chunking_threshold, min_chunk_size,
                max_chunk_size),
            chunks: Vec::new(),
            chunks_count: 0,
            chunked_bytes: 0u64,
    ...
    pub fn with_seed(chunking_threshold: u64, min_chunk_size: u64, max_chunk_size: u64, seed: u128) -> Self {
        Self{
            span_generator: SpanGenerator::with_seed(chunking_threshold, min_chunk_size,
                max_chunk_size, seed),
            chunks: Vec::new(),
            chunks_count: 0,
            chunked_bytes: 0u64,
    ...
    fn add_new_chunk(&mut self) -> &mut Chunk {
        let chunk = Chunk{
    ...
    fn update_chunk_readiness(&mut self) {
        ...
        self.chunks.iter_mut().rev().skip(2)
    ...
    pub fn adjust_last_chunks(&mut self) {
        ...
        self.chunks.iter_mut().take_while(|chnk| !chnk.is_ready())
                    .for_each(|chnk| chnk.set_is_ready(true));
        ...
        let pos_in_chunks = self.chunks.iter().rposition(|chnk| chnk.index() == first_changed_index).unwrap();
        let last_spans = self.span_generator.last_spans();
        let pos_in_spans = last_spans.iter().rposition(|sp| sp.index() == first_changed_index).unwrap();
        ...
        if  chunk_data_len as u64 > last_spans[pos_in_spans].size() {
            ...
            rchunk.data_mut().splice(0..0,
                lchunk.data_mut().split_off(chunk_data_len - surplus));
            ...
            if pos_in_chunks + 1 < self.chunks.len() {
                ...
                let lchunk= left_chunks.last_mut().unwrap();
                let rchunk = right_chunks.first_mut().unwrap(); 
                lchunk.data_mut().extend(rchunk.data_mut().drain(0..deficit));
            }
        }
        // @todo: what if last chunk size became 0 ???
        self.chunks.iter_mut().skip(pos_in_chunks).zip(last_spans.iter().skip(pos_in_spans))
