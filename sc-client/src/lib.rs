...
use rand::RngCore;
use sc_shared::{FileHeader, Metadata};
use web_sys::window;
use anyhow::Result;
use aes_gcm::{
    aead::{Aead, KeyInit},
...
fn do_encrypt_file(file_name: &str, data: &[u8], password: &str, file_header: &mut FileHeader)
    ...
    // Encrypt {serialized metadata}{padded_data}
    let mut to_encrypt = meta_data.to_bytes()
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn decrypt_file(data: &[u8], password: &str)
    ...
    let decrypted_data = cipher.decrypt(nonce, remaining_data)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    // decrypted_data is expected to start with metadata
    let (metadata, metadata_size) = Metadata::from_bytes_ex(&decrypted_data)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    // {metadata}{data}{padding}
    let file_data = decrypted_data[metadata_size..metadata_size+metadata.file_size as usize].to_vec();
